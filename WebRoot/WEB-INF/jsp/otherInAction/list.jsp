...
$(document).ready(function(e) {
	...
	for ( var j = 0; j < leng; j++ ) {
		...
		if($('#'+a).parent().parent().parent().find('tr').eq(j).find('td').eq(1).text()=='已审核'){
 		...
 		if($('#'+a).parent().parent().parent().find('tr').eq(j).find('td').eq(2).text()==$('#'+a).parent().parent().parent().find('tr').eq(j+1).find('td').eq(2).text()){
 			...
 			$('#'+a).parent().parent().parent().find('tr').eq(j+1).find('td').eq(11).css('visibility','hidden');
 		}
	}  
	/**
...
<div class="page-header">
  <div class="row">
    <div class="col-md-8">
      <h1>其他入库单管理<small>OtherIn InventoryIn Management</small></h1>
    </div>
    ...
    <s:form action="purchaseAction_search" class="form form-inline" role="form" method="post">
		<div class="form-inline">
 		<s:select name="condition" cssClass="form-control input-sm" id="condition"
			list="#{'date':'日期','id':'单据编号','checkyn':'审核标志','userId':'业务员','warehouseId':'收料仓库','materialsId':'物料名称'}" 
...
<div class="panel panel-info">
	<div class="panel-heading">其他入库单
		<div class="pull-right">
			<s:a action="otherInAction_addUI" cssClass="btn btn-default btn-sm" role="button">添加</s:a>
		</div>
	</div>
<table id="tbl" class="table table-hover table-bordered table-striped table-responsive">
  ...
  <s:iterator value="recordList">
    <tr>
      ...
      <td>${ number }</td>
      <td>${ totalPrice }</td>
      <td><s:a id="delete" action="otherInAction_delete?itemId=%{inventoryInItem.id}" onClick="return confirm('确定要删除吗?')" title="删除"><i class="fa fa-lg fa-trash-o"></i></s:a>
      	  <s:a id="edit" action="otherInAction_editUI?itemId=%{inventoryInItem.id}" title="修改"><i class="fa fa-pencil-square-o fa-lg"></i></s:a>
      	  <s:a id="audit" action="otherInAction_audit?itemId=%{inventoryInItem.id}" onClick="return confirm('确定要审核?')" title="审核"><i class="fa fa-lg fa-check-square-o"></i></s:a></td>
    </tr>
    </s:iterator>
  </tbody>
</table>
...
<s:form action="otherInAction_list"></s:form>
</body>
</html>
