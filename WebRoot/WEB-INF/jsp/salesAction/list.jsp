...
<div class="page-header">
  <div class="row">
    <div class="col-md-8">
      <h1>销售出库单管理<small>Sales InventoryOut Management</small></h1>
    </div>
    <div class="col-md-4" id="search">
      <s:form action="salesAction_search" class="form form-inline" role="form" method="post">
		<div class="form-inline">
 		<s:select name="condition" cssClass="form-control input-sm" id="condition"
			list="#{'date':'日期','id':'单据编号','checkyn':'审核标志','userId':'业务员','warehouseId':'发货仓库','materialsId':'货品名称','consumerId':'购货单位'}" 
...
<div class="panel panel-info">
	<div class="panel-heading">销售出库单
		<div class="pull-right">
			<s:a action="salesAction_addUI" cssClass="btn btn-default btn-sm" role="button">添加</s:a>
		</div>
	</div>
<table id="tbl" class="table table-hover table-bordered table-striped table-responsive">
  ...
  <s:iterator value="recordList">
    <tr>
      ...
      <td>${ inventoryOutItem.consumer.name }</td>
      ...
      <td>${ number }</td>
      <td>${ totalPrice }</td>
      <td><s:a id="delete" action="salesAction_delete?itemId=%{inventoryOutItem.id}" onClick="return confirm('确定要删除吗?')" title="删除"><i class="fa fa-lg fa-trash-o"></i></s:a>
          <s:a id="edit" action="salesAction_editUI?itemId=%{inventoryOutItem.id}" title="修改"><i class="fa fa-pencil-square-o fa-lg"></i></s:a>
          <s:a id="audit" action="salesAction_audit?itemId=%{inventoryOutItem.id}" onClick="return confirm('确定要审核?')" title="审核"><i class="fa fa-lg fa-check-square-o"></i></s:a></td>
    </tr>
    </s:iterator>
  </tbody>
</table>
