...
@Controller
@Scope("prototype")
public class ProductAction extends BaseAction<Object> {

	private Long itemId; // 产品入库单主表编号
	...
	public String search() throws Exception {
		...
		QueryHelper queryHelper = new QueryHelper(InventoryInMaterials.class,
				"iim")
				.addOrderByProperty("iim.id", false)
				.addWhereCondition("iim.inventoryInItem.inType=?",
						InventoryInItem.TYPE2)
		...
		if (condition.equals("departmentId") && !content.equals("")) {
			queryHelper.addWhereCondition(
					"iim.inventoryInItem.department.name like ?",
					"%" + content.trim() + "%");
			PageBean pageBean = inventoryInMaterialsService.getPageBean(
					pageNum, pageSize, queryHelper);
			ActionContext.getContext().getValueStack().push(pageBean);
	...
	public String add() throws Exception {
		...
		inventoryInItem.setCheckyn(InventoryInItem.CHECKN);
		inventoryInItem.setInType(InventoryInItem.TYPE2);
		inventoryInItem.setDepartment(departmentService.getById(departmentId));
	...
	public String editUI() throws Exception {
		...
		if (inventoryInItem.getDepartment() != null) {
			departmentId = inventoryInItem.getDepartment().getId();
		}
	...
	public String audit() throws Exception {
		...
		if (inItem.getCheckyn().equals("未审核")) {
			...
			inItem.setChecker(BaseAction.getCurrentUser().getName());
